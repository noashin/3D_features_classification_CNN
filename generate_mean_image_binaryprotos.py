...
@click.option('--arguments_file', type=click.STRING,
              ...
              help='arguments list')
@click.option('--results_path', type=click.STRING,
              default='/home/noa/pcl_proj/experiments/mean_image_files/all_instances',
              help='path to save the lmdb file')
@click.option('--hist', type=click.BOOL, default=False)
@click.option('--index', type=click.INT,)
def main(arguments_file, results_path, hist, index):
    save_prototype_files(arguments_file, results_path, hist, index)
